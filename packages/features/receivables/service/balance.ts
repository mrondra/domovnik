...
export const unitBalance = async (ctx: RequestContext, input: UnitBalanceInput): Promise<UnitBalance> =>
  (await receivablesAdapterFor(ctx, input.svjId)).unitBalance(ctx, input);
...
): Promise<readonly UnitBalance[]> => (await receivablesAdapterFor(ctx, input.svjId)).listDebtors(ctx, input);
