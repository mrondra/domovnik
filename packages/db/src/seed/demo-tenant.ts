...
import type { SeedContext } from '../../../kernel/src/seed/index';
...
/**
 * The kernel-owned rows every feature seed then writes into: the demo tenant, one user per role and
 * an identity for every registered agent. Keyed by name, e-mail and agent name + version, so it is
 * safe to re-run.
 */
export const seedDemoTenant = async (): Promise<SeedContext> => {
